...
from datasets import build_classification_dataloaders
from models import Task2ClassifierNet
from utils import (
    AverageMeter,
    accuracy,
...
def train(config_path: str):
    ...
    print(f"  label_smooth  = {cfg['train'].get('label_smoothing', 0.0)}")
    ...
    print("\nfinish")
    print(f"[Result] best_acc = {best_acc:.4f} (epoch {best_epoch})")
    print(f"[Result] best checkpoint = {ckpt_path}")


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="configs/task2.yaml")
